...
def textual_train(train_dataloader, clip_model, optimizer, args, device):
    total_loss = []
    ...
    for i, items in enumerate(tqdm(train_dataloader)):
        ...
        loss = focal_loss(predict_labels, labels) + args.lambda1 * (focal_loss(predict_masks, gts) + l1_loss(predict_masks, gts)) + args.lambda2 * patch_alignment_loss(img_feature, labels, gts)
        optimizer.zero_grad()
...
def visual_train(train_dataloader, clip_model, optimizer, args, device):
    total_loss = []
...
def train(args=None):
    ...
    with wandb.init(project=f"miniCLIP-AD-{args.run_name}", config=args):      
        ...
        learnable_params, textual_params_dict, visual_params_dict = clip_model.insert(tokenizer=tokenize, device=device)
        ...
        if args.mode == 'test':
            ...
            clip_model.cls_adapter.load_state_dict(visual_ckpt['cls_adapter'])
            #clip_model.patch_adapter.load_state_dict(visual_ckpt['patch_adapter'])
            ...
            for epoch in range(1, args.epochs + 1):
                if joint == 1:
                    ...
                    t_loss = textual_train(
                        train_dataloader, clip_model, optimizer, args, device
                    )
                    v_loss = visual_train(
                        train_dataloader, clip_model, optimizer, args, device
                    )
                    total_loss = (np.mean(t_loss) + np.mean(v_loss)) / 2
                    ...
                    logger.info(
                        "Epoch: {}/{}, Loss: {:.6f}, T_Loss: {:.6f}, V_Loss: {:.6f}, LR: {:.8f}".format(
                        epoch, args.epochs, np.mean(total_loss), np.mean(t_loss), np.mean(v_loss), optimizer.param_groups[0]['lr']
                    ))
