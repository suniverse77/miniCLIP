...
class Attention_Adapter(nn.Module):
    ...
    def forward(self, img_token):
        x = img_token + self.attention(img_token)

        return x
        

class CLS_Projector(nn.Module):
    ...
    def forward(self, img_token):
        out = self.shortcut(img_token) + self.mlp(img_token)
...
class Text_Adapter(nn.Module):
    ...
    def forward(self, img_token):
        out = self.ln(self.mlp(img_token))
